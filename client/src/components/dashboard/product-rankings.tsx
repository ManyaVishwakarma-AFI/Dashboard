...
//   const BASE_URL = "http://122.176.108.253:9001"; // Remote server IP
    ...
    >
      <div className="flex items-center space-x-3">
        ...
        <div>
          <p className="font-medium text-sm">{product.product_title.replace(/"/g, '')}</p>
          <p className="text-xs text-muted-foreground">
            {product.review_count} reviews • {product.avg_rating.toFixed(1)} ⭐
          </p>
        </div>
      </div>
      <TrendingUp className="h-5 w-5 text-green-600" />
    ...
    const fetchTrendingProducts = async () => {
      ...
      try {
        ...
        if (filters && (filters.category !== "All Categories" || filters.rating > 0)) {
          ...
          console.log("Filtered trending products:", analyticsData);

          if (analyticsData && analyticsData.top_products) {
            setTrending(analyticsData.top_products);
          }
        } else {
          // No filters - fetch normal trending
          const res = await fetch(`${BASE_URL}/top?table=amazon_reviews&n=10`);
          const json = await res.json();
          if (json && json.data) {
            setTrending(json.data);
      ...
      <Card className="bg-card rounded-xl p-6 border hover:shadow-md transition-shadow">
        <CardHeader className="flex flex-row items-center justify-between mb-4 p-0">
          <CardTitle className="text-lg font-semibold">
            ...
            {filters && filters.category !== "All Categories" && (
