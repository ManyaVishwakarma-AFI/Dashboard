...
export default function FiltersPanel({ selectedSource }: { selectedSource: string }) {
  ...
  //     setFilters(prev => ({ ...prev, category: "All Categories" })); // reset category when table changes
  //   } catch (err) {
  ...
  try {
    ...
    // Reset category only if the previous category does not exist in the new list
    setFilters(prev => {
